...
localparam INPUT_BITWIDTH = 31;
localparam OUTPUT_BITWIDTH = 17;
		...
		if (valid_out_hyperb) begin
			...
			if (scalefactor_latch_out != 0) begin
				...
				if (scalefactor_latch_out < 0)
					temp_result = temp_result >>> -scalefactor_latch_out;
				else if (scalefactor_latch_out > 0)
					temp_result = temp_result <<< scalefactor_latch_out;
			end

			temp_result = temp_result >>> 34 ;
			if (temp_result[0] == 1)
				temp_result = temp_result + 1;
			temp_result = temp_result >>> 1;

			result = temp_result[16:0];
